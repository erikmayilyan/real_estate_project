...
if (isset($_POST['form_submit'])) {
  try {
    ...
    $statement = $conn->prepare("SELECT * FROM property_videos WHERE property_id=?");
    $statement->execute(array($_REQUEST['id']));
    $total_videos = $statement->rowCount();
    if ($total_videos == $allowed_videos) {
      throw new Exception("You have reached your maximum limit of videos. Please upgrade you rpackage to upload more videos or delete existing videos to add new videos.");
    };
    ...
    $statement = $conn->prepare("INSERT INTO property_videos (property_id, video_id) VALUES (?, ?)");
    $statement->execute(array($_REQUEST['id'], $_POST['video_id']));
...
if (isset($_POST['form_delete'])) {
  $statement = $conn->prepare("DELETE FROM property_videos WHERE id=?");
            ...
            <div class="col-lg-9 col-md-12">
                ...
                <div class="video-all">
                    <?php
                        ...
                        if (!$total) {
                          echo '<div class="col-md-12 text-danger"><p class="text-center">No video added yet.</p></div>';
                        }
