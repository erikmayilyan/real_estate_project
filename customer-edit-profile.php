...
if (isset($_POST['form_update'])) {
  try {
    ...
    $statement = $conn->prepare("SELECT * FROM customers WHERE email=? AND id!=?");
    ...
    $statement = $conn->prepare('UPDATE customers SET full_name=?, email=? WHERE id=?');
    ...
    if ($_POST['password'] != '' || $_POST['retype_password'] != '') {
      if ($_POST['password'] != $_POST['retype_password']) {
        ...
        $statement = $conn->prepare('UPDATE customers SET password=? WHERE id=?');
    ...
    if ($path != '') {
      ...
      if ($mime == 'image/jpeg' || $mime == 'image/png') {
        ...
        $statement = $conn->prepare('UPDATE customers SET photo=? WHERE id=?');
